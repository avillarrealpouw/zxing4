...
public final class PDF417Writer implements Writer {
  ...
  private static BitMatrix bitMatrixFromEncoder(PDF417 encoder,
    ...
    System.out.println("height "+height+" width "+width+" originalScale[0].length"
        +originalScale[0].length+" originalScale.length "+originalScale.length);
    //if ((height > width) != (originalScale[0].length < originalScale.length)) {
    //  originalScale = rotateArray(originalScale);
    //  rotated = true;
    //  System.out.println("The matrix will be rotated!!!");
    //}
